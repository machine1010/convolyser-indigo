...
# ==================== RESULTS ====================
    ...
    tab1, tab2 ,tab3 = st.tabs(["📝 Transcription","Transcription_table", "📈 Analysis"])
    ...
    with tab2:
        st.markdown("### Transcription Output Table")
        ...
        if st.session_state.transcription_raw:
            ...
            if not tdf.empty:
                ...
                st.info("No transcript table segments found.")
    
            # Keep the existing download button
            with open(st.session_state.transcription_path, "rb") as f:
                st.download_button(
                    label="⬇️ Download Transcription Table JSON",
                    data=f.read(),
                    file_name="transcription_table.json",
                    mime="application/json"
                )
