...
class AudioAnalysisPipeline:
    """
    Streamlined pipeline for Hindi audio transcription, evaluation, analysis,
    and comparison with agent-provided JSON data.
    """
    
    def __init__(self, credentials_path: str, project_id: str = "gen-lang-client-0155506887", location: str = "us-central1"):
        """
        Initialize the pipeline with Google Cloud credentials.
        
        Args:
            credentials_path: Path to Google Cloud service account JSON key
            project_id: Google Cloud project ID (optional, reads from env if not provided)
...
You are an expert quality analyst tasked with evaluating whether a survey agent properly asked all required questions during a Hindi language audio call. Your job is to analyze the provided transcript and determine if each survey question was asked correctly by the agent.
...
You are an expert semantic analysis system tasked with comparing two sets of survey answers in Hindi language. Your job is to evaluate whether two answers to the same question are semantically equivalent, partially equivalent, or completely different.
...
'''
    ...
    def _load_audio_to_base64(self, file_path: str) -> Tuple[str, str]:
        """Load audio file and return base64 encoded data with mime type."""
    ...
    def _load_json_to_base64(self, file_path: str) -> str:
        """Load JSON file and return base64 encoded data."""
    ...
    def _save_output(self, content: str, output_path: str, clean: bool = True) -> None:
        """Save content to file with optional cleaning."""
        if clean:
    ...
    def process_audio(
        ...
        evaluation_path = os.path.join(output_dir, "evaluation_output.json")
        analysis_path = os.path.join(output_dir, "audio_analysis.json")
        merged_path = os.path.join(output_dir, "merged_survey.json")
        comparison_path = os.path.join(output_dir, "comparison_output.json")
        final_path = os.path.join(output_dir, "final_output.json")

        # Step 1: Transcribe audio
        print("Step 1/6: Transcribing audio...")
        audio_data, mime_type = self._load_audio_to_base64(audio_file_path)
        response = self.model.generate_content(
            [{'mime_type': mime_type, 'data': audio_data}, self.transcription_prompt],
            generation_config=self.generation_config
        )
        self._save_output(response.text, transcript_path)
        ...
        response = self.model.generate_content(
            [{'mime_type': 'text/plain', 'data': transcript_b64}, self.evaluation_prompt],
            generation_config=self.generation_config
        )
        self._save_output(response.text, evaluation_path)

        # Step 3: Analyze transcript content
        print("Step 3/6: Analyzing transcript...")
        response = self.model.generate_content(
            [{'mime_type': 'text/plain', 'data': transcript_b64}, self.analysis_prompt],
            generation_config=self.generation_config
        )
        self._save_output(response.text, analysis_path)
        ...
        response = self.model.generate_content(
            [{'mime_type': 'text/plain', 'data': merged_b64}, self.comparison_prompt],
            generation_config=self.generation_config
        )
        self._save_output(response.text, comparison_path)
        ...
        # Load JSON content
        for key in ['transcript', 'evaluation', 'analysis', 'merged', 'comparison', 'final']:
...
def run_pipeline(audio_path: Path, agent_json_path: Path, license_path: Path) -> Tuple:
    """
    Convenience function to run the pipeline with Path objects.
    
    Args:
        audio_path: Path to audio file
        agent_json_path: Path to agent JSON file
        license_path: Path to credentials file
        
    Returns:
        Tuple of (final_path, final_content, all_results)
    """
    credentials_path = str(license_path)
    out_dir = tempfile.mkdtemp(prefix="audio-analysis-")
    ...
    return result['final_path'], result['final'], result
